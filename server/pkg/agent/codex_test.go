...
func TestCodexHandleResponseUnknownIDIgnored(t *testing.T) {
	t.Parallel()
	c, _, _ := newTestCodexClient(t)
	// No pending entry for id 99 — should not panic
	c.handleLine(`{"jsonrpc":"2.0","id":99,"result":{"ok":true}}`)
}
...
func TestCodexHandleServerRequestUnknownMethodEmptyResult(t *testing.T) {
	t.Parallel()
	c, fs, _ := newTestCodexClient(t)
	c.handleLine(`{"jsonrpc":"2.0","id":12,"method":"unknown/method","params":{}}`)
	lines := fs.Lines()
	if len(lines) != 1 {
		t.Fatalf("expected 1 response, got %d", len(lines))
	}
	...
	if _, ok := resp["result"].(map[string]any); !ok {
		t.Fatalf("expected empty result map, got %v", resp["result"])
	}
}
...
func TestCodexLegacyEventPatchApply(t *testing.T) {
	t.Parallel()
	c, _, _ := newTestCodexClient(t)
	var messages []Message
	c.onMessage = func(msg Message) { messages = append(messages, msg) }
	c.handleLine(`{"jsonrpc":"2.0","method":"codex/event","params":{"msg":{"type":"patch_apply_begin","call_id":"p1"}}}`)
	c.handleLine(`{"jsonrpc":"2.0","method":"codex/event","params":{"msg":{"type":"patch_apply_end","call_id":"p1"}}}`)
	if len(messages) != 2 {
		t.Fatalf("expected 2 messages, got %d", len(messages))
	}
	if messages[0].Type != MessageToolUse || messages[0].Tool != "patch_apply" {
		t.Fatalf("unexpected: %+v", messages[0])
	}
	if messages[1].Type != MessageToolResult || messages[1].Tool != "patch_apply" {
		t.Fatalf("unexpected: %+v", messages[1])
	}
}
...
func TestCodexInt64Aliases(t *testing.T) {
	t.Parallel()
	m := map[string]any{"input_tokens": float64(0), "input": float64(42)}
	if got := codexInt64(m, "input_tokens", "input", "prompt_tokens"); got != 42 {
		t.Fatalf("expected 42 from alias input, got %d", got)
	}
}

func TestCodexExtractUsageFromMap(t *testing.T) {
	t.Parallel()
	c := &codexClient{}
...
func TestCodexExtractUsageFromTokenUsageField(t *testing.T) {
	t.Parallel()
	c := &codexClient{}
	c.extractUsageFromMap(map[string]any{
